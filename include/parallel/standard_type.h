#ifndef __JETS_PAR_STANDARD_TYPE_H__
#define __JETS_PAR_STANDARD_TYPE_H__

#include "jetscfg.h"
#include "data_type.h"
#include "semipermanent.h"
#include "standard_type_forward.h"
// C++ includes.
#include <array>
#include <complex>
#include <list>
...
#define JETS_STANDARD_TYPE(cxxtype, mpitype)                         \
  template<>                                                         \
...
JETS_STANDARD_TYPE(char, MPI_CHAR);
JETS_STANDARD_TYPE(signed char, MPI_SIGNED_CHAR);
JETS_STANDARD_TYPE(unsigned char, MPI_UNSIGNED_CHAR);
JETS_STANDARD_TYPE(short int, MPI_SHORT);
JETS_STANDARD_TYPE(unsigned short int, MPI_UNSIGNED_SHORT);
JETS_STANDARD_TYPE(int, MPI_INT);
JETS_STANDARD_TYPE(unsigned int, MPI_UNSIGNED);
JETS_STANDARD_TYPE(long, MPI_LONG);
JETS_STANDARD_TYPE(long long, MPI_LONG_LONG_INT);
JETS_STANDARD_TYPE(unsigned long, MPI_UNSIGNED_LONG);
JETS_STANDARD_TYPE(unsigned long long, MPI_UNSIGNED_LONG_LONG);
JETS_STANDARD_TYPE(float, MPI_FLOAT);
JETS_STANDARD_TYPE(double, MPI_DOUBLE);
JETS_STANDARD_TYPE(long double, MPI_LONG_DOUBLE);
...
// them and we don't like to leak them, so let's keep them until JETS
...
template<>
class StandardType<JETS_DEFAULT_SCALAR_TYPE> : public DataType
{
public:
    explicit
        StandardType(const JETS_DEFAULT_SCALAR_TYPE* =nullptr) : DataType()
    {
        static data_type static_type=MPI_DATATYPE_NULL;
        if (static_type == MPI_DATATYPE_NULL)
        {
            jets_call_mpi(MPI_Type_contiguous(2, MPI_DOUBLE, &static_type));
            SemiPermanent::add
    ...
    StandardType(const StandardType<JETS_DEFAULT_SCALAR_TYPE>& t) : DataType()
    {
...
#else
# ifdef JETS_DEFAULT_QUADRUPLE_PRECISION
JETS_STANDARD_TYPE(JETS_DEFAULT_SCALAR_TYPE, );
# endif
#endif
        ...
        : DataType()
    {
        ...
        if (static_type == MPI_DATATYPE_NULL)
        {
          // We need an example for MPI_Address to use
            ...
            static_assert(N > 0, "Zero-length std::array is not supported by JETS");
...
#endif // __JETS_PAR_STANDARD_TYPE_H__